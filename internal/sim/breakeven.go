package sim

const (
	breakEvenCapDays = 600
	probeLow         = 1_000
	probeHigh        = 2_000
	maxProbeCash     = Coins(1) << 40
)

type CyclePlan struct {
	Days       int64
	Mass       Micrograms
	PricePerKg Coins
	BreakEven  FishCount
}

func (b *Balance) CycleAt(kind TankKind, at Tick, zone ZoneOffset) CyclePlan {
	low, ok := probeCycle(b, kind, at, zone, probeLow)
	if !ok {
		return CyclePlan{}
	}

	high, ok := probeCycle(b, kind, at, zone, probeHigh)
	if !ok {
		return low.CyclePlan
	}

	perFish := (high.margin - low.margin) / (probeHigh - probeLow)
	if perFish <= 0 {
		return low.CyclePlan
	}

	fixed := probeLow*perFish - low.margin
	plan := low.CyclePlan
	plan.BreakEven = FishCount(min(max((fixed+perFish-1)/perFish, 0), maxInt32))

	return plan
}

type probe struct {
	CyclePlan

	margin int64
}

func probeCycle(b *Balance, kind TankKind, at Tick, zone ZoneOffset, fish int64) (probe, bool) {
	s := NewState(1, zone, at)
	s.Cash = maxProbeCash

	tank, ok := s.AddTank(kind, b.Tanks[kind].Litres)
	if !ok {
		return probe{}, false
	}
	s.StockTank(tank, FishCount(fish), b.Growth.FingerlingMass, Coins(int64(b.Economy.FingerlingPrice)*fish))
	s.SeedOxygen(b)
	s.Tanks[0].Upgrades = 1<<AutoFeeder | 1<<AutoAerator

	var best probe

	for day := range int64(breakEvenCapDays) {
		out, err := Advance(Input{State: s, Until: at + Tick(day+1)*TicksPerDay, Balance: b})
		if err != nil {
			return probe{}, false
		}
		s = out.State

		if s.TankCount == 0 || s.Tanks[0].BatchCount == 0 {
			return probe{}, false
		}
		batch := s.Tanks[0].Batches[0]
		price := b.PriceFor(batch.MeanMass, s.Tick)
		value := mulDivFloor(int64(price), int64(batch.Biomass()), int64(MicrogramsPerKilogram))

		if margin := value - int64(batch.Cost); margin > best.margin || best.Days == 0 {
			best = probe{
				CyclePlan: CyclePlan{
					Days:       day + 1,
					Mass:       batch.MeanMass,
					PricePerKg: price,
				},
				margin: margin,
			}
		}

		if batch.MeanMass >= topClass(b) {
			break
		}
	}

	return best, best.Days > 0
}

func topClass(b *Balance) Micrograms {
	var top Micrograms
	var best PPM

	for i := range b.Market.ClassCount {
		class := b.Market.Classes[i]
		if class.PPM > best {
			best, top = class.PPM, class.UpToMass
		}
	}
	if top == 0 {
		return b.Growth.HarvestMass
	}

	return top
}
